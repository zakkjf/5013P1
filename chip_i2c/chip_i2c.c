 ...
 **/
static const unsigned short normal_i2c[] = {0x38, 0x39, 0x40, I2C_CLIENT_END };
...
struct chip_data {
    ...
    unsigned long sensor_last_read; /* In jiffies */
    int kind;
 ...
 */
static int chip_read_mult(struct i2c_client *client, u8 reg, u8 len, u8 *buf)
{
    int ret;
...
/**
 * Read a single byte via I2C using "repeated start" condition.
 *
 * Two I2C messages are being sent by this function:
 *   1. I2C write operation (write register address) with no STOP bit in the end
 *   2. I2C read operation
 ...
 * @client: I2C client structure
 * @reg: register address (subaddress)
 * @buf: buffer which will contain read byte
 *
 * Returns 0 on success or negative value on error.
 */
static int chip_read_value(struct i2c_client *client, u8 reg, u8 *buf)
{
	return chip_read_mult(client, reg, 1, buf);
}
        ...
        size_t count, loff_t * offset)
{
    ...
    for (x = 0; x < count; x++)
        if (chip_write_value(chip_i2c_client, REG_CHIP_PORTA_LOUT, (u8)tmp[x]) == 0)
            numwrite++;

    return numwrite;
}

// Our file op read function
static ssize_t chip_i2c_read(struct file * fp,  char __user * buf,
        size_t count, loff_t * offset)
{
    int x, numread = 0;
    char * tmp;

    /* We'll limit the number of bytes read in */
    if (count > 512)
        count = 512;

    tmp = kmalloc(count,GFP_KERNEL);

    printk("%s: Read operation with [%d] bytes\n", __FUNCTION__, (int) count);
    for (x = 0; x < count; x++)
        if (chip_read_value(chip_i2c_client, REG_CHIP_PORTA_LOUT, (u8*)(tmp+x)) == 0)
            numread++;
...
static ssize_t get_light_level(struct device *dev, 
...
static DEVICE_ATTR(chip_led, 0440, NULL, set_chip_led);
// chip switch is read only
static DEVICE_ATTR(light_level, 0220, get_light_level, NULL);
*/
